...
import java.util.Set;
	...
	public class OkHttpClientManager
	{
	     ...
	     * @param callback
	     */
	    private void _downloadAsyn(final String url, final String destFileDir,final String fileName,  final ResultCallback callback)
	    {
	        final Request request = new Request.Builder()
	                .url(url)
	                .build();
	        ...
	        call.enqueue(new Callback()
	        {
	            ...
	            public void onResponse(Response response)
	            {
	                ...
	                {
	                    is = response.body().byteStream();
	                    File file = new File(destFileDir, getFileName(fileName));
	                    fos = new FileOutputStream(file);
	                    while ((len = is.read(buf)) != -1)
	                    {
	                        fos.write(buf, 0, len);
...
//	    }
	    ...
	    public static void downloadAsyn(String url, String destDir,String fileName, ResultCallback callback)
	    {
