...
import cn.wochu.wh.decorator.MyDivider;
...
public class ThridOrderSearchAct extends BaseActivity {
    ...
    private List<ThridOrderSearchEntity.DCJDORDERDETAILItemBean> list;
    ...
    protected void handleCode(String str) {
        if (CheckCodeUtil.isWochuGoodsNum(str)) {
            /**
             * 商品编码
             */
...
//            test(str);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initView();
        initData();
        wharehouseID = SharePreUtil.getInteger(this, Constants.SP_ACCOUNT_LOGON_WAREHOUSEID, 0);
    }
    /**
     * 扫描后变色
     * @param str
...
//        ToastCheese(wharehouseID+"");
        OkHttpClientManager.getAsyn(ApiClient.getDCJDOrderInfo(orderNum,wharehouseID+""), new OkHttpClientManager.ResultCallback<BaseEntity<ThridOrderSearchEntity>>() {
            @Override
...
//                ToastCheese(response.toString());
    ...
    private void initView() {
        ...
        rvMainPage.addItemDecoration(new MyDivider(this,MyDivider.VERTICAL_LIST));
        butGoodsOffSale.setEnabled(false);
    }
    ...
    private void test(String orderNum){

        ToastCheese(wharehouseID+"");
        OkHttpClientManager.getAsyn(ApiClient.getDCJDOrderInfo(orderNum, wharehouseID + ""), new OkHttpClientManager.ResultCallback<String>() {
            @Override
            public void onError(Request request, Exception e) {
